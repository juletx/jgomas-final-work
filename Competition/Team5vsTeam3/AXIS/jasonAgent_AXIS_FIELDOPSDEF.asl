...
+!perform_aim_action
    <-  // Aimed agents have the following format:
        ...
        if (AimedAgentTeam == 100) {
            ...
            /*.concat("goto", NewDestination, BroadcastPlan );
            .my_team("AXIS", ListaCompas);

            .length(ListaCompas, ListaCompasLength);

            +bucleC(0);
            while (bucleC(Xc) & (Xc < Length)){
                .nth(Xc, ListaCompas, Compa);
                .send(Compa, achieve, pos(NewDestination));
                -+bucleC(Xc + 1);
            }
            -bucleC(_);*/

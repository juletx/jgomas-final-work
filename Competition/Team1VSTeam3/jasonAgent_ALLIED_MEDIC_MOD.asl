 ...
 +!setup_priorities
        <-  
            ...
            +task_priority("TASK_GIVE_AMMOPAKS", 0); // Give ammo
    ...
    */
    ...
    // New beliefs are created storing the soldier which corresponds to each medic
	// This is done to ease the subgroup task
    
        .my_name(Me);
        .term2string(Me, S);
        if (S = "A21"){
            //Medic 1 with soldier 1
            +my_soldier("A11");
        }else{
            if (S = "A22"){
                //Medic 2 with soldier 2
                +my_soldier("A12");
            }else{
                +my_soldier("None");
            }

...
/* 
 ?my_position(A, B, C);
 !add_task(task(4000, "TASK_GIVE_MEDICPACKS", "Manager", pos(A, B, C+2), ""));
 .println("Me estoy dando un pack médico");
 */
 ?my_health(Hr);
...
+!init
   <- ?debug(Mode); if (Mode<=1) { .println("YOUR CODE FOR init GOES HERE.")}
    ...
    .concat(TaskList, [task(2002, "TASK_GOTO_POSITION", M, pos(233, 0, 108), "")], NewT0);           
    +tasks(NewT0);
    .concat(TaskList, [task(2002, "TASK_GOTO_POSITION", M, pos(181, 0, 54), "")], NewT1);           
    +tasks(NewT1);   
  	.concat(TaskList, [task(2002, "TASK_GOTO_POSITION", M, pos(128, 0, 118), "")], NewT2);           
    +tasks(NewT2);
    .concat(TaskList, [task(2002, "TASK_GOTO_POSITION", M, pos(166, 0, 147), "")], NewT3);           
    +tasks(NewT3);

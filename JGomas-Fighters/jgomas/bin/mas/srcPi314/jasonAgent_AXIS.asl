...
+!perform_aim_action
    <-  // Aimed agents have the following format:
        ...
        if (AimedAgentTeam == 100) {
			...
			//!add_task(task(750, "TASK_GO_TO_POSITION", M, NewDestination, ""));
			
			?tasks(Tasklist);
			.delete(task(_, "TASK_PATROLLING", M, _, _),Tasklist,NewTaskList);
			.delete(task(_, "TASK_GO_TO_POSITION", M, _, _),NewTaskList,NewTaskList2);
			-+tasks(NewTaskList2);
			!add_task(task(750, "TASK_GO_TO_POSITION", M, NewDestination, ""));
			/*
			?objective(ObjectiveX, ObjectiveY, ObjectiveZ);
...
+!setup_priorities
    <-  +task_priority("TASK_NONE",0);
        ...
        +task_priority("TASK_PATROLLING", 20);
